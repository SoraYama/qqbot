...
const helpText = {
  ...
  order: `向其他提督交换资源和舰娘, 后面的指令有:
s2r | r2s | r2r | s2s: s=ship, r=resource, 即指定交易类型, 后面参数为交易的数额, 例如: "/fleet s2r 1001,1000-20000,20000,20000,20000 1551" 代表交出ID为1001和1000的舰娘换取用户ID为1551的20000各项资源 (注意: 逗号分隔不能有空格, 用户ID不指定则谁都可以交易)
list: 查看现在在交易中的ID
me: 可以查询自己发布和接受的订单ID
